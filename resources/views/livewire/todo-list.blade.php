        ...
        <header id="header" class="border-b border-indigo-600">
            <div x-data class="flex flex-row items-stretch bg-white">
                <div class="flex items-center bg-gray-100">
                    <svg wire:click="updateTodosStatusOnCurrentPage('{{ json_encode(collect($todos->items())->map->id->toArray()) }}')"
                        class="w-8 h-8 mx-4 cursor-pointer fill-current @if($checkItemsOnCurrentPage) text-gray-800 @else text-gray-500 @endif"
            ...
            <ul id="todo-list" class="list-none">
                ...
                <li wire:key="li-{{ $todo->id }}" x-data="initTodo({{ json_encode($todo) }})"
                    ...
                    <div class="flex flex-row items-stretch bg-white">
                        <div class="flex items-center bg-gray-100">
                            <svg wire:click="updateTodoStatus({{ $todo->id }}, '{{ $todo->status }}')"
                                class="@if($todo->isCompleted()) text-green-500 @endif w-8 h-8 mx-4 cursor-pointer fill-current"
                                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                <path
                        ...
                        <div class="flex items-center">
                            <svg class="absolute right-0 w-8 h-8 mr-4 text-gray-800 transition duration-300 ease-out transform cursor-pointer fill-current hover:scale-125 hover:text-red-500"
